...
body { background-color: #ffffff; 
        background-image: url("folketing.jpg");
...
<body>
    ...
    <div id="dom-target_PT" style="display: none;">
        ...
        // clamp date to limits
        if($year < 2007 && $month < 2)
        {
           $year = 2007; $month = 02; $day = 2-1; 
        }
        
        if($year < 2007)
        {
           $year = 2007; $month = 10; $day = 2-1; 
        }
        ...
        do
        {
            ...
            #$URL0 = sprintf('http://demo.cratica.org/sessoes/%d/%02d/%02d/', $year, $month, $day );
               ...
               $URLlocal = sprintf('DK/dk_%04d-%02d-%02d.txt', $year, $month, $day );
        ...
        //$result = str_replace("Dr.", 'Dr', $result);
        //$result = str_replace("Abg.", 'Dr', $result);
        ...
        $output = 0;
                ...
                $text = str_ireplace('ministeren', '',$text);
                $text = str_ireplace('danmark', '',$text);
                $text = str_ireplace('eller', '',$text);
                $text = str_ireplace('nogle', '',$text);
                $text = str_ireplace('synes', '',$text);
                $text = str_ireplace('stformand', '',$text);
                $text = str_ireplace('kunne', '',$text);
                $text = str_ireplace('bliver', '',$text);
                $text = str_ireplace('meget', '',$text);
                $text = str_ireplace('kommer', '',$text);
                $text = str_ireplace('deres', '',$text);
                $text = str_ireplace('flere', '',$text);
                $text = str_ireplace('gerne', '',$text);
                $text = str_ireplace('noget', '',$text);
    ...
    <div id="backdiv" >
    <br>
        <div style='font-size:60px; font-family: helvetica, sans-serif;' id="titulo">
        <img height=40px src="https://upload.wikimedia.org/wikipedia/commons/9/9c/Flag_of_Denmark.svg"> Radio Folketing <img height=40px src="https://upload.wikimedia.org/wikipedia/commons/9/9c/Flag_of_Denmark.svg">
        </div>
    <br>
    <h2 style="margin: 0; font-size:30px" id="data"></h2>
        <a href="./index.dk.php" style="margin: 0; font-size: 14px; text-decoration: underline;">Tilfældig dato</a>
        <a id="escolherData" style="margin: 0; margin-left: 1em; font-size: 14px; text-decoration: underline;">Vælge en dato</a>
        ...
        <h4 style="margin: 0; font-weight: bold; font-size: 24px;">Oversætter</h4>
            <select id="lang" onclick="setTimeout(setEN,2500)" style="margin: 0;">
                  <option value="ar;Arabic Female">عربى/Arabic</option>
                  <option value="zh-TW;Chinese Female">中文/Kinesisk</option>
                  <option value="ko;Korean Female">한국어/Korean</option>
                  <option value="cs;Czech Female">čeština/Tjekkisk</option>
                  <option value="de;Deutsch Female">Deutsch</option>
                  <option value="da;Danish Female">Dansk/Dänisch</option>
                  <option selected="selected" value="en;UK English Female">English</option>
                  <option value="es;Spanish Female">Español</option>
                  <option value="el;Greek Female">Ελληνικά/Græsk</option>
                  <option value="fr;French Female">Français</option>
                  <option value="hi;Hindi Female">हिंदी/Hindi</option>
                  <option value="hu;Hungarian Female">Magyar/Ungarsk</option>
                  <option value="it;Italian Female">Italiano</option>
                  <option value="ja;Japanese Female">日本語/Japansk</option>
                  <!-- <option value="la;Latin Female">Latim</option> <!-- Latin crashes some browsers -->
                  <option value="nl;Dutch Female">Nederlands</option>
                  <option value="pl;Polish Female">Polskie/Polsk</option>
                  <option value="pt;Portuguese Female">Português</option>
                  <option value="pt-BR;Brazilian Portuguese Female">PT Brasileiro</option>
                  <option value="ro;Romanian Male">Română/Rumænsk</option>
                  <option value="ru;Russian Female">русский/Russisk</option>
                  <option value="sv;Swedish Female">Svenska/Svensk</option>
                  <option value="fi;Finnish Female">Suomi/Finsk</option>
                  <option value="no;Norwegian Female">Norsk/Norsk</option>
                  <option value="tr;Turkish Female">Türk/Tyrkisk</option>
                  <option value="th;Thai Female">ไทย/Thai</option>
                </select>
        ...
        <a id="linkDemocracia" href="http://demo.cratica.org" style="margin: 20px;">Fuld Transkript</a> 
        <br> <br>
        ...
        <div style="max-width:400px; margin-top: 50px;">
            ...
            <div id="footer">Folketingets udskrifter høflighed af <a id="afooter" href="http://www.ft.dk">ft.dk</a></div>
            <div id="footer">Multilingual Automatisk oversættelse med <a id="afooter" href="https://translate.google.com/">google translate</a></div>
            <div id="footer">Multilingual automatisk aflæsning med <a id="afooter" href="https://responsivevoice.org/">responsivevoice.js</a></div>
            <div id="footer">Tag cloud oprettet med <a id="afooter" href="https://http://mistic100.github.io/jQCloud/">jQCloud.js</a></div>
            <div id="footer">CC2 billede baggrund høflighed af  <a id="afooter" href="https://en.wikipedia.org/wiki/Folketing#/media/File:Christiansborg_Folketingssalen_20120420_0222F_(8188480571).jpg">News Oresund & wikipedia</a></div>
            <div id="footer">Virker bedst på Mac og iOS, de har langt bedre TTS stemmer.</div>
            <!-- <div id="footer">Produção <a id="afooter" href="expressao.xyz">expressao.xyz</a></div> -->
            <div id="footer">Få kildekoden på <a id="afooter" href="https://github.com/fsilva/radio-parlamento">GitHub</a></div> 
            <br>
            <button style="font-size: 12px" onclick=" $('#BRvoice').show();$('#maindiv').hide();">Skift Læs dansk tekst sprog.</button>
    ...
    <form method="get" id="dateForm" action="index.dk.php">
...
<!--    <div id='novoice' style='display: none;text-align:center;'><div style="margin:10px; center:"><h2>:( Ihr Computer unterstützt nicht das automatische Lesen (Text-To-Speech) :( </ h2>
        
         <H6> Und unsere Abgeordneten werden diese Bedingungen nicht aussprechen! Überprüfen Sie Ihre Staatshaushalt , Premierminister! <I> Applaus </ i> </ h6>
         Filmographie
         <H3> Wir empfehlen Ihnen, das Radio-Parlament versuchen, auf einem Computer mit MacOS oder iOS (Portugiesisch von Portugal) oder Windows oder Android (Portugiesisch in Brasilien). </ H3>
         <H5> nicht unterstützten Web-Browser: Internet Explorer / Microsoft Kante </ h5>
        </div></div>-->
    
    <div id='BRvoice' style='display: none;text-align:center;'><div style="margin:10px; center:"><h2>Ja</h2> 
        
        <h4>To use best use Radio Parliament please use a computer with macOS 10.10, iOS. </h4>
        <h5> Unsupported Web-Browsers: Internet Explorer/Microsoft Edge</h5>
        
        <h2> Or select a different voice to read Danish text:</h2>
        ...
        <button style="font-size:20px; width:50%; margin: 5px;" onclick='init("Deutsch Female")'> German </button><p></p>
    ...
    <script type="text/javascript">
        ...
        var words = [];
        var items = [];
        var tts_language = 'Danish Female';
        ...
        function    init(dictation_language)
        {
            ...
            var start = new Date("2007-10-02");
            var end = new Date("2017-03-03");
            $('#escolherData_div').datepicker({yearRange: "2007:2017", dateFormat: 'yy-mm-dd', changeYear: true, changeMonth: true, onSelect: function () {dateSelected();}});
        ...
        function updateLine(items,ii)
        {
            ...
            var url = "https://translate.googleapis.com/translate_a/single?client=gtx&sl=da&tl="+lang+"&dt=t&q=" +encodeURI(textPT);
            var xmlHttp = new XMLHttpRequest();
        ...
        {   
            ...
            var url = "https://translate.googleapis.com/translate_a/single?client=gtx&sl=da&tl="+lang+"&dt=t&q=" +encodeURI(totrans);
            var xmlHttp = new XMLHttpRequest();
            xmlHttp.open( "GET", url, false ); // false for synchronous request
            xmlHttp.send( null );
            var textEN =  xmlHttp.responseText;
